...
# Orca's canonical worktree id is "<repo id>::<absolute worktree path>" and is
# the only token Orca resolves, so records carry it whole and every addressing
# call passes it through unmodified. Validation checks that exact shape.
fm_backend_orca_worktree_ref_valid() {  # <value>
  ...
  case "$backend" in
    ...
    orca)
      ...
      if [ "$window" != "fm-$id" ] \
        ...
        || ! fm_backend_orca_worktree_ref_valid "$worktree_id"; then
