...
When queued wake rows already exist, that bounded nag appends the shared queued-wake repair line from `bin/fm-supervision-instructions.sh --repair-line --queue-pending 1` so the handoff is not reported as outage-only.
With an empty queue, the same bounded nag remains outage-only and does not invent a drain instruction.
In Calm mode, a `check: rearm-resurface` follow-up is suppressed only when the queue is empty; if queued work exists, that re-ring is still delivered because it is the only delivery path for those queued rows.
