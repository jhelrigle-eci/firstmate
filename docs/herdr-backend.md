...
Projected workspaces publish `fm_name=└ <concise-task>` through `workspace report-metadata --token fm_name=<name>` so Spaces can show a clean child row while the branch row remains native.
