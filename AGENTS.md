...
state/               runtime records and signals; gitignored
  ...
  .afk-contract      the away-posture record: the captain's verbatim away words, expected return, reach profile, spend cap, structured mandate clauses, and the declared `away` or `quiet` mode that is the only mode signal where no .afk flag is written; written only by bin/fm-afk-contract.sh after the captain confirms the read-back, archived under afk-contracts/ at return; its presence IS the away posture in every harness, while a quiet declaration keeps ordinary captain work flowing under it; its sibling .afk-contract.lock serializes actions authorized by the live record (contract: bin/fm-afk-contract.sh)
