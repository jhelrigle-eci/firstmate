...
state/               runtime records and signals; gitignored
  ...
  .afk-contract      the away-posture record: the captain's verbatim away words, expected return, reach profile, spend cap, structured mandate clauses, and the declared `away` or `quiet` mode that is the only mode signal where no .afk flag is written; written only by bin/fm-afk-contract.sh after the captain confirms the read-back, archived under afk-contracts/ at return; its presence IS the away posture in every harness, while a quiet declaration lets the away-return guard pass for reporting and ordinary chat and leaves merge and decision authority unchanged; its sibling .afk-contract.lock serializes actions authorized by the live record (contract: bin/fm-afk-contract.sh)
...
Firstmate is a bash agent-orchestration template, not a compiled application: supervisor tooling in `bin/` plus Markdown contracts, with no build step, and its runtime is the long-running background watcher process `bin/fm-watch.sh` rather than a deployed service.
Section 1 lists the shared tracked material, and the tracked tree also holds documentation in `docs/`, suites in `tests/`, and harness-integration code in `.pi/extensions/`, `.omp/extensions/`, and `.opencode/plugins/`; the block above owns the private operational-home layout.
